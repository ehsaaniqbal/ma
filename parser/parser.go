...
type Parser struct {
	...
	errors []string     // keeps track of all the errors
...
func (p *Parser) parseFunctionLiteral() ast.Expression {
	...
	literal.Parameters = p.parseFunctionParamaters()
...
// Advances the tokens if expected token type is correct
func (p *Parser) expectPeek(t token.TokenType) bool {
	if p.peekTokenIs(t) {
		p.nextToken()
		return true
	} else {
		p.appendError(t, "PeekError")
		return false
	}
}
...
func (p *Parser) appendError(t token.TokenType, errorType string) {
	var msg string
